...
// Get pet data
$petQuery = "SELECT p.*, c.name as category_name, s.seller_id, s.business_name, s.rating, 
            u.county, u.user_id as seller_user_id
            FROM pets p
            JOIN categories c ON p.category_id = c.category_id
            JOIN seller_profiles s ON p.seller_id = s.seller_id
            JOIN users u ON s.user_id = u.user_id
            WHERE p.pet_id = $petId AND p.approval_status = 'approved'";
$petResult = $conn->query($petQuery);

// Check if pet exists
if ($petResult->num_rows === 0) {
    setFlashMessage('error', 'Pet not found or not approved');
    redirect('buyer/browse.php');
}
...
$wishlistQuery = "SELECT * FROM wishlist_items WHERE user_id = $userId AND item_type = 'pet' AND item_id = $petId";
$wishlistResult = $conn->query($wishlistQuery);
$inWishlist = $wishlistResult->num_rows > 0;

// Check if in cart
$cartQuery = "SELECT * FROM cart_items WHERE user_id = $userId AND item_type = 'pet' AND item_id = $petId";
$cartResult = $conn->query($cartQuery);
$inCart = $cartResult->num_rows > 0;

// Get pet images
$imagesQuery = "SELECT * FROM images WHERE item_type = 'pet' AND item_id = $petId ORDER BY is_primary DESC";
...
$otherPetsQuery = "SELECT p.*, 
                  ...
                  FROM pets p
                  WHERE p.seller_id = {$pet['seller_id']} 
                  AND p.pet_id != $petId 
...
$relatedPetsQuery = "SELECT p.*, 
                    (SELECT image_path FROM images WHERE item_type = 'pet' AND item_id = p.pet_id AND is_primary = 1 LIMIT 1) as image
                    FROM pets p
                    WHERE p.category_id = {$pet['category_id']} 
                    AND p.pet_id != $petId 
                    AND p.status = 'available' 
                    AND p.approval_status = 'approved'
                    LIMIT 3";
...
$reviewsQuery = "SELECT r.*, u.first_name, u.last_name, u.profile_image
                ...
                WHERE r.item_type = 'pet' AND r.item_id = $petId 
                AND (r.status = 'approved' OR r.status = 'pending') 
                ORDER BY r.created_at DESC";
$reviewsResult = $conn->query($reviewsQuery);

// Calculate average rating - also include pending reviews
$avgRatingQuery = "SELECT AVG(rating) as avg_rating, COUNT(*) as review_count 
                  FROM reviews 
                  WHERE item_type = 'pet' AND item_id = $petId 
...
$userReviewQuery = "SELECT * FROM reviews WHERE user_id = $userId AND item_type = 'pet' AND item_id = $petId";
$userReviewResult = $conn->query($userReviewQuery);
$hasReviewed = $userReviewResult->num_rows > 0;

// Increment view count
$updateViewsQuery = "UPDATE pets SET views = views + 1 WHERE pet_id = $petId";
$conn->query($updateViewsQuery);

// Log activity
logActivity('view_pet', ['pet_id' => $petId]);
        ...
        <div class="col-lg-9">
            <nav aria-label="breadcrumb" class="mb-4">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><?php echo $pet['name']; ?></li>
                </ol>
            </nav>
                ...
                <div class="card-body p-0">
                    <div class="row g-0">
                        ...
                        <div class="col-md-6">
                                    ...
                                    <div class="carousel-inner">
                                        ...
                                        ?>
                                            <div class="carousel-item <?php echo $active ? 'active' : ''; ?>">
                                                <img src="<?php echo  '../' . $image['image_path']; ?>" class="d-block w-100" alt="<?php echo $pet['name']; ?>" style="height: 400px; object-fit: cover;">
                                            </div>
                                            <?php $active = false; ?>
                                        <?php endwhile; ?>
                                    </div>
                            ...
                            <?php else: ?>
                                <img src="<?php echo BASE_URL; ?>assets/images/pet-placeholder.jpg" class="img-fluid w-100" alt="<?php echo $pet['name']; ?>" style="height: 400px; object-fit: cover;">
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="p-4">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <div>
                                        <span class="badge bg-primary mb-2"><?php echo $pet['category_name']; ?></span>
                                        <?php if ($pet['featured']): ?>
                                            <span class="badge bg-warning mb-2">Featured</span>
                                        <?php endif; ?>
                                        <h2><?php echo $pet['name']; ?></h2>
                                    </div>
                                    <button class="btn btn-sm <?php echo $inWishlist ? 'btn-danger' : 'btn-outline-danger'; ?>" id="wishlistBtn" data-id="<?php echo $petId; ?>">
                                ...
                                <div class="mb-3">
                                    ...
                                    <p class="text-muted">
                                        <i class="fas fa-map-marker-alt"></i> <?php echo $pet['county']; ?> &nbsp;|&nbsp;
                                        <i class="fas fa-eye"></i> <?php echo $pet['views']; ?> views
                                    </p>
                                </div>
                                
                                <h4 class="text-primary mb-3">KES <?php echo number_format($pet['price'], 2); ?></h4>
                                
                                <div class="mb-3">
                                    <div class="row">
                                        <div class="col-6 mb-3">
                                            <p class="text-muted mb-1">Breed</p>
                                            <p class="fw-bold"><?php echo $pet['breed']; ?></p>
                                        </div>
                                        <div class="col-6 mb-3">
                                            <p class="text-muted mb-1">Age</p>
                                            <p class="fw-bold"><?php echo $pet['age']; ?></p>
                                        </div>
                                        <div class="col-6 mb-3">
                                            <p class="text-muted mb-1">Gender</p>
                                            <p class="fw-bold"><?php echo ucfirst($pet['gender']); ?></p>
                                        </div>
                                        <div class="col-6 mb-3">
                                            <p class="text-muted mb-1">Quantity Available</p>
                                            <p class="fw-bold"><?php echo $pet['quantity']; ?></p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-grid gap-2 mb-3">
                                    ...
                                    <?php else: ?>
                                        <button class="btn btn-primary" id="addToCartBtn" data-id="<?php echo $petId; ?>">
                                            <i class="fas fa-shopping-cart me-2"></i> Add to Cart
                                        </button>
                                    <?php endif; ?>
                                    
                                    <button class="btn btn-outline-primary" id="contactSellerBtn" data-id="<?php echo $pet['seller_user_id']; ?>">
                                ...
                                <div class="seller-info p-3 bg-light rounded">
                                    <div class="d-flex align-items-center mb-2">
                                        ...
                                        <div>
                                            <h6 class="mb-0"><?php echo $pet['business_name']; ?></h6>
                                            <div class="text-warning">
                                                <?php
                                                $sellerRating = $pet['rating'];
                                                for ($i = 1; $i <= 5; $i++) {
                                                    if ($i <= $sellerRating) {
                                        ...
                                        <a href="<?php echo BASE_URL; ?>buyer/seller_profile.php?id=<?php echo $pet['seller_id']; ?>" class="btn btn-sm btn-outline-primary ms-auto">View Profile</a>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addReviewModalLabel">Write a Review for <?php echo $pet['name']; ?></h5>
            ...
            <form id="reviewForm" action="<?php echo BASE_URL; ?>buyer/submit_review.php" method="post">
                <div class="modal-body">
                    ...
                    <input type="hidden" name="item_id" value="<?php echo $petId; ?>">
        ...
        <div class="modal-content">
            ...
            <form id="messageForm" action="<?php echo BASE_URL; ?>buyer/send_message.php" method="post">
                <div class="modal-body">
                    ...
                    <input type="hidden" name="related_to_item_type" value="pet">
                    <input type="hidden" name="related_to_item_id" value="<?php echo $petId; ?>">
                    
                    <div class="mb-3">
                        <label for="messageSubject" class="form-label">Subject</label>
                        <input type="text" class="form-control" id="messageSubject" name="subject" value="Inquiry about <?php echo $pet['name']; ?>" required>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $(document).ready(function() {
        ...
        $('#addToCartBtn').click(function() {
            ...
            // Debug - check if we're getting the correct pet ID
            console.log('Adding pet ID to cart:', petId);
                ...
                success: function(response) {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            // Show success toast
                            showToast('Success', 'Pet added to your cart successfully', 'success');
        ...
        $('#wishlistBtn').click(function() {
            ...
            // Debug - check if we're getting the correct pet ID
            console.log('Toggling wishlist for pet ID:', petId);
